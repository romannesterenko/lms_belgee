...
cmi.learner_name = "";  // Read only
cmi.learner_id = "";    // Read only
cmi.location = "";      // Read and write
cmi.launch_data = "";   // Read only
cmi.suspend_data = "";  // Read and write
cmi.entry = "ab-initio";    // Read only 
cmi.credit = "credit";  // Read only   "credit", "no-credit"
cmi.mode = "normal";    // Read only   "browse", "normal", "review"
cmi.max_time_allowed = 0;   // Read only
cmi.time_limit_action = "continue,no message";  // Read only   "exit,message", "continue,message", "exit,no message", "continue no message"
//cmi.completion_status = "incomplete";   // Read and write   "completed", "incomplete", "not attempted", "unknown"
cmi.completion_threshold = 0.0; // Read only
cmi.success_status = "unknown"; // Read and write   "passed", "failed", "unknown"
cmi.progress_measure = 0.0;     // Read and write
cmi.total_time = 0;     // Read only - seconds
cmi.scaled_passing_score = 0.0; // Read only
cmi.score_min = 0.0;    // Read and write
cmi.score_max = 0.0;    // Read and write
cmi.score_scaled = 0.0; // Read and write
cmi.score_raw = 0.0;    // Read and write
cmi.pref_audio = 0.0;   // Read and write
cmi.pref_lang = "";     // Read and write
cmi.pref_speed = 0.0;   // Read and write
cmi.pref_caption = 0;   // Read and write

cmi.exit = "_none_";    // Write only   "time-out", "suspend", "logout", "normal", ""  - "_none_" is used to detect change to ""
cmi.session_time = 0;   // Write only - seconds

cmi.interactions = new Array();
var cmi_interaction = new Object(); // Read and write
cmi_interaction.id = "";
...
cmi.comments = new Array();     // Read and write
cmi.lms_comments = new Array(); // Read only
var cmi_comment = new Object();
cmi_comment.comment = "";
cmi_comment.location = "";
cmi_comment.timestamp = 0;      // UTC seconds

cmi.objectives = new Array();   // Read and write
    ...
    if (cmi.lms_init_url != '') {
        ...
        var xhr = new XMLHttpRequest();
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        //console.log("LMS connection response: " + xhr.responseText);
                        data = xhr.responseText;
                      } else {
                        //console.error("LMS connection error: " + xhr.statusText);
                        result = false;
                      }     
                };
            };
    ...
    if (cmi.lms_commit_url != '') {
        ...
        var xhr = new XMLHttpRequest();
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                        if (xhr.status === 200) {
                        // /console.log("SEND TO LMS: " + xhr.responseText);
...
cmi.formatData = function() {
    var result = "";
    var items = new Array();
    items.push("\"user_id\":" + $('#user_id').val());
    items.push("\"course_id\":" + $('#course_id').val());
    items.push("\"part\":" + $('#part').val());
    ...
    if(cmi.scorm_version=='1.2')
        items.push("\"lesson_status\":" + _json_quote(cmi.lesson_status));
