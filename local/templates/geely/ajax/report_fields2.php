...
$already_op = [];
$already_ppo = [];
if(check_full_array($request['city'])){
    foreach ($request['city'] as $city){
        foreach(\Models\Dealer::getByCity($city, ['ID', 'PROPERTY_REGIONAL', 'PROPERTY_REGIONAL_PPO']) as $d) {
            $not_show_ids = [2, 4714];
