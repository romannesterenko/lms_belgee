...
<div class="authorization-content">
    ...
    <div class="authorization">
        ...
        <div class="authorization__form">
            <span class="authorization__title"><?=GetMessage("AUTH_AUTHORIZE")?></span>
            <?php
            if(!empty($arParams["~AUTH_RESULT"])){
                $text = str_replace(array("<br>", "<br />"), "\n", $arParams["~AUTH_RESULT"]["MESSAGE"]);?>
                <div style="padding: 25px 0px; color: red; font-weight: 500;"><?=nl2br(htmlspecialcharsbx($text))?></div>
            <?php }
            if($_REQUEST['type_no_login']=='no_dealer'){?>
                <div style="padding: 25px 0px; color: white; text-decoration: underline; font-weight: 500;">Вы не привязаны к дилеру, обратитесь к администратору</div>
            <?php }?>
            <form name="form_auth" method="post" action="<?=$arResult["AUTH_URL"]?>">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="" name="USER_LOGIN" value="<?=$arResult["LAST_LOGIN"]?>" />
                </div>
                <div class="form-group">
                    <label for=""><?=GetMessage("AUTH_PASSWORD")?></label>
                    <div class="password">
                        <input class='validate form-control' type="password" name="USER_PASSWORD" id='confirm-password'/>
                ...
                <span class="forgot-password"><a href="?forgot_password=yes"><?=GetMessage("AUTH_RESTORE_PASSWORD")?></a></span>
                <div class="btn-center">
                    <button class="btn btn--white"><?=GetMessage("AUTH_TO_AUTHORIZE")?></button>
                </div>
