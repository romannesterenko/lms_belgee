    ...
    <div class="authorization">
        ...
        <div class="authorization__form">
            ...
            if(!empty($arParams["~AUTH_RESULT"])){
                ...
                <div class="alert <?=($arParams["~AUTH_RESULT"]["TYPE"] == "OK"? "alert-success":"alert-danger")?>" style="padding: 15px 0px;color: #fff;"><?=nl2br(htmlspecialcharsbx($text))?></div>
            <?php }else{?>
                <p><?=GetMessage('AUTH_FORGOT_TEXT');?></p>
            <?php }?>
            <form name="bform" method="post" action="<?=$arResult["AUTH_URL"]?>">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="soloviev@yandex.ru" name="USER_EMAIL" value="<?=$arResult["USER_EMAIL"]?>" />
                </div>

                <div class="btn-center">
                    <button name="send_account_info" type="submit" class="btn btn--white"><?=GetMessage("AUTH_RESTORE")?></button>
                </div>
