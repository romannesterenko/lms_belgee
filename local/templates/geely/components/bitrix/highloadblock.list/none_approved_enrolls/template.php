...
<div class="content-block content-block--margin">
    ...
    <div class="event-info">
        <span class="event-info__item"><?= Loc::getMessage('NON_APPR_ENROLLS_DATE') ?><span class="event-info__item-date"><?= DateHelper::getHumanDate($arResult['SCHEDULE']['PROPERTIES']['BEGIN_DATE'], 'd F')?></span></span>
        <span class="event-info__item"><?= Loc::getMessage('NON_APPR_ENROLLS_FORMAT') ?><span
                    class="event-info__item-offline"><?=$arResult['COURSE']['PROPERTIES']['COURSE_FORMAT']?></span></span>
        <span class="event-info__item"><?= Loc::getMessage('FOR_WHOM') ?><span><?= Loc::getMessage('FOR') ?><?=implode(' / ', $arResult['ROLES']);?></span></span>
    </div>
    ...
    <div class="applications">
        <div class="applications__nav">
            <?php if(CSite::InDir('/cabinet/confirmation/new/')){?>
                <span class="active"><a href="#"><?= Loc::getMessage('NEW') ?></a></span>
            <?php }else{?>
                <span><a href="/cabinet/confirmation/new/<?=$arResult['SCHEDULE']['ID']?>/"><?= Loc::getMessage('NEW') ?></a></span>
            <?php }?>
                ...
                <div class="application-item">
                    ...
                    <?php if(!$arResult['SCHEDULE']['ENDED']){?>
                        <div class="application-item__btns">
                            <a href="#" class="btn btn--small course_application_actions" data-action="confirm" data-id="<?=$row['ID']?>">
                                <svg class="icon" width="18px" height="10px">
