...
class CabinetSettingsShowmaterialsComponent extends CBitrixComponent
{
    /**
    ...
    {
        $settings = new \Settings\ShowMaterials();
        $exists = $settings->getExistSetting();
        $this->arResult['ITEMS'] = $settings->getAllValues()->getArray();
        foreach($this->arResult['ITEMS'] as &$value){
            if(empty($exists))
                $value['CHECKED'] = $value['DEF']=='Y'?'checked':'';
            else
                $value['CHECKED'] = in_array($value['ID'], $exists)?'checked':'';
        }
    }
    /**
