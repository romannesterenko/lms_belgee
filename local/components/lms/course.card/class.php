...
class CourseCardComponent extends CBitrixComponent
{
                    ...
                    foreach ($this->enrolls as $enroll) {
                        if ($enroll['UF_IS_APPROVED'] == 1 && $enroll['UF_DIDNT_COM'] != 1) {
                            if ($enroll['UF_FAILED'] == 1){
                                ...
                                if(!SheduleCourses::isExistsCheckedByCourse($this->item['ID'])) {
    ...
    {
        $this->item['CURRENT_COMPLETION'] = current((new \Teaching\CourseCompletion())
            ->get([
                'UF_COURSE_ID' => $this->item['ID'],
                'UF_USER_ID' => \Helpers\UserHelper::prepareUserId(0),
                'UF_IS_COMPLETE' => false,
                'UF_FAILED' => false,
            ]));
        if(check_full_array($this->item['CURRENT_COMPLETION'])&&$this->item['CURRENT_COMPLETION']['UF_SHEDULE_ID']>0){
            $this->item['CURRENT_SHEDULE'] = current(SheduleCourses::getById($this->item['CURRENT_COMPLETION']['UF_SHEDULE_ID']));
        }
