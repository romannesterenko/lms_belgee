...
class CabinetCoursesEnrolledComponent extends CBitrixComponent
{
    ...
    protected function getResult()
    {
        $schedule_ids = [];
        $items = [];
        $enrolled = new \Teaching\CourseCompletion();
        foreach ($enrolled->getAllStartedCompletionsByUser() as $item) {
            if ($item['UF_COURSE_ID']>0)
                $item['COURSE'] = \Teaching\Courses::getById($item['UF_COURSE_ID']);
            if($item['UF_SHEDULE_ID']>0)
                $item['SCHEDULE'] = \Teaching\SheduleCourses::getById($item['UF_SHEDULE_ID']);
            $items[] = $item;
        }
        $this->arResult = [
            'ITEMS' => $items,
