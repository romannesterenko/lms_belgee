...
class Completion
{
    ...
    {
        $sections = IBlock::getSections(['IBLOCK_ID' => IBlock::getCompletionIblock(), 'UF_COURSE' => $course_id], ['UF_*']);
    ...
    {
        return current(IBlock::getElements(['IBLOCK_ID' => IBlock::getCompletionIblock(), 'SECTION_ID' => $completion_id]));
    }

    public static function getAllStages($completion_id)
    {
        return IBlock::getElements(['IBLOCK_ID' => IBlock::getCompletionIblock(), 'SECTION_ID' => $completion_id], ['SORT' => 'ASC', 'ID' => 'ASC']);
    }

    public static function getCourseID($ID)
    {
        $completion_array = current(IBlock::getSections(['IBLOCK_ID' => IBlock::getCompletionIblock(), 'ID' => $ID], ['UF_*']));
