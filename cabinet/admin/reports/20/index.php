...
$dealers = \Models\Dealer::getAll(['ID', 'NAME', 'CODE', 'PROPERTY_REGIONAL']);
unset($dealers[360]);
$regionals = [];
/*foreach ($dealers as &$dealer) {
    if((int)$dealer['PROPERTY_REGIONAL_VALUE']>1)
        $regionals[$dealer['PROPERTY_REGIONAL_VALUE']] = \Models\User::getFullName($dealer['PROPERTY_REGIONAL_VALUE']);
}*/
?>
    ...
    <div class="main-content">
        ...
        <div class="content">
            <div class="content-block">
                <h2 class="h2 center" style="margin-bottom:10px">Баланс дилеров</h2>
                ...
                <div class="form-div">
                    <form class="report_generator" action="report.php" method="post" style="">
                        ...
                        <div class="form-group selectable">
                            <label for="">Региональный менеджер</label>
                            <select class="js-example-basic-multiple" name="regional_op[]" multiple="multiple" style="width: 100%;">
                                <?php foreach ($regionals as $id => $regional_name){?>
                                    <option value="<?=$id?>"<?=$_REQUEST['regional']&&in_array($id, $_REQUEST['regional_op'])?' selected':''?>><?=$regional_name?></option>
                                <?php }?>
                            </select>
                        </div>
                        <div class="form-group selectable">
                            ...
                            <select class="js-example-basic-multiple" name="dealer" style="width: 100%;">
                                ...
                                <?php foreach ($dealers as $dealer_id => $dealer_array){?>
                                    <option value="<?=$dealer_id?>"><?=$dealer_array['NAME']?></option>
                                <?php }?>
                            </select>
                        </div>
    ...
    <script>
        ...
        function processData(main_entity){
            ...
            $.ajax({
                type: 'POST',
                url: '/local/templates/geely/ajax/report_fields20.php',
                data: formData,
                processData: false,
                contentType: false,
                dataType: 'json',
                ...
                success: function(response){
                    ...
                    if (response.dealers && response.dealers.length > 0) {
                        $('[name="dealer"]').append('<option value="all">Все дилеры</option>');
                        for (var k in response.dealers) {
                            ...
                            $('[name="dealer"]').append('<option ' + selected_string + ' value="' + response.dealers[k].ID + '">' + response.dealers[k].NAME + '</option>');
                        }
                        $('[name="dealer"]').select2({language: "en"});
                    }
