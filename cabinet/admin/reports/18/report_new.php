...
if (!empty($_REQUEST['from'])) {
    $shedule_filter['>=PROPERTY_BEGIN_DATE'] = $_REQUEST['from'] . ' 00:00:00';
}
            ...
            foreach ($questions_data as $iKeyTheme => $question_one) {
                ...
                foreach ($question_one['QUESTIONS'] as $iKeyQuestion => $q) {
                    foreach ($answers as $answer) {
                        if ($arCompareQuestionsByOld[$q['ID']]){
                            $q['ID']= $arCompareQuestionsByOld[$q['ID']];
                        }
                        if ($arCompareQuestionsByOld[$answer['UF_QUESTION_ID']]){
                            $answer['UF_QUESTION_ID'] = $arCompareQuestionsByOld[$answer['UF_QUESTION_ID']];
                        }
                        if ((int)$answer['UF_QUESTION_ID'] == (int)$q['ID']) {

                            $common_values[112][$q['ID']][] = $r_data[$q['ID']] = (int)$answer['UF_POINTS'];
                ...
                $common_values[112]['PERCENTS'][] = $r_data['PERCENTS'] = round($common_score / $need_score * 100);

                $common_values[112]['COMMON'][] = $r_data['COMMON'] = round($common_score / count($question_one['QUESTIONS']), 1);
                $table_row['ANSWERS'][112] = $r_data;
            }
           // var_dump($r_data);
            //die();
...
?>
                                    ...
                                    <?php if (check_full_array($_REQUEST['questions'])) { ?>
                                        <?php foreach ($questions_data as $questions_datum) { ?>
                                            <?php foreach ($questions_datum['QUESTIONS'] as $one_question) {
                                                ...
                                                if (empty($item['ANSWERS'][112][$one_question['ID']]) && $reversAnswer) {
                                                    ?>
                                                    <td><?= $reversAnswer ?></td>
                                                <? } else {
                                                    ?>
                                                    <td><?= $item['ANSWERS'][112][$one_question['ID']] ?></td>
                                                <? } ?>
                                            <?php } ?>
                                        <?php }
                                    } else { ?>
                                        <?php foreach ($questions_data as $questions_datum) { ?>
                                            <td><?= number_format($item['ANSWERS'][112]['COMMON'], 1, ',') ?></td>
                                            <td><?= number_format($item['ANSWERS'][112]['PERCENTS'], 1, ',') ?>
                                                %
                                            </td>
                                            <?php foreach ($questions_datum['QUESTIONS'] as $one_question) { ?>
                                                <td><?= $item['ANSWERS'][112][$one_question['ID']] ?></td>
                            ...
                            if (check_full_array($table_data)) {
                                ...
                                <tr>
                                    ...
                                    <?php if (check_full_array($_REQUEST['questions'])) { ?>
                                        <?php foreach ($questions_data as $questions_datum) { ?>
                                            <?php foreach ($questions_datum['QUESTIONS'] as $one_question) {
                                                if ($arCompareQuestionsByOld[$one_question['ID']]){
                                                    $one_question['ID'] = $arCompareQuestionsByOld[$one_question['ID']];
                                                }
                                                if (!in_array($one_question['ID'], $_REQUEST['questions'])|| in_array($arCompareQuestionsByNew[$one_question['ID']] , $_REQUEST['questions'])) continue; ?>
                                                <td><?= number_format(array_sum($common_values[112][$one_question['ID']]) / count($common_values[112][$one_question['ID']]), 1, ',') ?></td>
                                            <?php } ?>
                                        <?php }
                                    } else { ?>
                                        <?php foreach ($questions_data as $questions_datum) { ?>
                                            <td><?= number_format(array_sum($common_values[112]['COMMON']) / count($common_values[112]['COMMON']), 1, ',') ?></td>
                                            <td><?= number_format(array_sum($common_values[112]['PERCENTS']) / count($common_values[112]['PERCENTS']), 1, ',') ?>
                                                %
                                            </td>
                                            <?php foreach ($questions_datum['QUESTIONS'] as $one_question) { ?>
                                                <td>
                                                    <? if (empty($common_values[112][$one_question['ID']])) {
                                                        echo ' </td>';
                                                        continue;
                                                    }
                                                    if ($arCompareQuestionsByOld[$one_question['ID']]){
                                                    ...
                                                    ?>
                                                    <?= number_format(array_sum($common_values[112][$one_question['ID']]) / count($common_values[112][$one_question['ID']]), 1, ',') ?>
                                                </td>
                                            <?php } ?>
                                        <?php }
                                    } ?>
    ...
    <script>console.log(<?=json_encode($common_values)?>)</script>
