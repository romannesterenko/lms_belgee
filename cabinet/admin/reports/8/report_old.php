...
$qwerty = [];
$users = [];
\Helpers\Log::write(['user' => $USER->GetID(), 'report' => '8', 'request' => $_REQUEST]);
$_REQUEST['report_id'] = 999999;
    ...
    $rsUsers = CUser::GetList(($by="personal_country"), ($order="desc"), $user_filter, ['FIELDS' => ['ID', 'NAME', 'LAST_NAME', 'EMAIL', 'PERSONAL_MOBILE', 'WORK_POSITION'], 'SELECT' => [ 'UF_ROLE', 'UF_DEALER', 'UF_REQUIRED_COURSES']]); // выбираем пользователей
    ...
    if (check_full_array($_REQUEST['fio'])){
        $filter_completions['UF_USER_ID'] = $_REQUEST['fio'];
    }
    if($_REQUEST['op_servis_op']=='on'||$_REQUEST['op_servis_servis']=='on') {
    ...
    $courses = \Models\Course::getList(['ACTIVE' => 'ALL', 'ID' => $course_ids], ['ID', 'NAME', 'CODE', 'ACTIVE', 'PROPERTY_SCORM', 'PROPERTY_COURSE_TYPE']);
    ...
    foreach($completions as &$completion){
        ...
        $completion['UF_REQUIRED_COURSES'] = $users[$completion['UF_USER_ID']]['UF_REQUIRED_COURSES'];
        ...
        if(!empty($completion['UF_DATE'])&&time()<$completion['UF_DATE']->getTimestamp()){
            $completion['ENROLLED'] = 1;
        }
        ...
        foreach ($users as $user) {
            foreach ($courses as $course) {
                ...
                if (!check_full_array($all_user_array[$user['ID'] . "_" . $course['ID']])) {
                    if($course['ACTIVE']!='Y')
                        continue;
                    if($need_setted_courses&&in_array($course['ID'], $setted_courses_ids)){
                        /*if(!check_full_array($user['UF_REQUIRED_COURSES']))
                            continue;
                        if(check_full_array($user['UF_REQUIRED_COURSES'])&&!in_array($course['ID'], $user['UF_REQUIRED_COURSES']))
                            continue;*/
                    }
                    ...
                    $completion['UF_REQUIRED_COURSES'] = $user['UF_REQUIRED_COURSES'];
