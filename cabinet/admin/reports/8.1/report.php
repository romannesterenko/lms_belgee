                    ...
                    <table class="table table-bordered table-striped table-responsive-stack table--borders" id="table-report">
                        ...
                        <?php foreach ($new_array as $item){
                            $first_item = current($item);
                                switch ($_REQUEST['direction']) {
                                    case "S01":
                                        if (empty(Dealer::getRegionalOP($first_item['DEALER']['ID'])))
                                            continue 2;
                                        break;
                                    case "A01":
                                        if (empty(Dealer::getRegionalPPO($first_item['DEALER']['ID'])))
                                            continue 2;
                                        break;
                                    case "M01":
                                        if (empty(Dealer::getRegionalMarketing($first_item['DEALER']['ID'])))
                                            continue 2;
                                        break;
                                    /*case 'all':
                                        if (
                                            empty(Dealer::getRegionalOP($first_item['DEALER']['ID'])) &&
                                            empty(Dealer::getRegionalPPO($first_item['DEALER']['ID'])) &&
                                            empty(Dealer::getRegionalMarketing($first_item['DEALER']['ID']))
                                        ) {
                                            continue 2;
                                        }
                                        break;*/
