...
$course = \Models\Course::find($current_completion['UF_COURSE_ID'], ['NAME']);
$current_question = [];
$questions_count = \Models\FeedbackPoll::getAllQuestions();
$number = count($questions_count);
if($_REQUEST['sended'] == 'Y') {
    $url = '/cabinet/courses/feedback_poll/'.(int)$_REQUEST['form_completion'].'/';
...
if(ProcessFBPOll::isRunning($current_completion['ID'])) {
    if(\Models\FeedbackPoll::isFinished($current_completion['ID'])) {
        ...
        $current_question = \Models\FeedbackPoll::getNextQuestion($current_completion['ID']);
    }
} else {
    $current_question = \Models\FeedbackPoll::getNextQuestion($current_completion['ID']);
}
                        ...
                        <div class="poll-item" style="display: flex; justify-content: space-between">
                            <?php foreach (range(1, 5) as $point) {?>
                                <div class="radio-item">
                                    <input type="radio" value="<?=$point?>" id="poll-question-<?=$point?>" name="points" checked/>
